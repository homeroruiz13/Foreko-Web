...
const Shader: React.FC<ShaderProps> = ({ source, uniforms, maxFps = 60 }) => {
  ...
  if (hasError) {
    return <div className="absolute inset-0 h-full w-full bg-gradient-to-t from-gray-950 to-[84%]" />;
  }

  return (
    <ErrorBoundary onError={() => setHasError(true)}>
      <DynamicCanvas className="absolute inset-0  h-full w-full">
...
> {
  ...
  componentDidCatch(error: any, errorInfo: any) {
    console.error('Error in Shader component:', error, errorInfo);
    this.props.onError?.();
  }
